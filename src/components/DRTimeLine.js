	...
	render() {
		return (
			<div className="dr-timeline">
				<div className="dr-timeline-point dr-timeline-cursor"></div>
				...
				{this.props.ranges.map((range, index) => {
					...
					return <div key={key} className="dr-timeline-range" style={{left: startPos, width: width, backgroundColor: color}}></div>
				})}
			</div>
		);
	}
